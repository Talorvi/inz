  ...
  <div>
    ...
    <q-dialog v-model="dialog">
      <q-card>
        ...
        <q-card-section> <b>Type:</b> {{ type }} </q-card-section>
        <q-card-section> <b>Script:</b> {{ script }} </q-card-section>
      </q-card>
    </q-dialog>
    ...
    goToNew() {
      this.$router.push(
        "/game/" +
          ...
          "/gameManagement/abilities/languages/new",
        () => {}
      );
    },
    getCustomFeatures() {
    ...
    showFeatureDialog(name) {
      ...
      axios
        ...
        .then(response => {
          ...
          this.script = response.data[0].script;
          this.dialog = true;
        })
        .catch(error => {
          if (error.response.status === 401) {
