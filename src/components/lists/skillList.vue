  ...
  <div>
    ...
    <q-dialog v-model="dialog">
      <q-card>
        ...
        <q-card-section>
          <b>Ability score:</b> {{ abilityScore }}
        </q-card-section>
    ...
    goToNew() {
      this.$router.push(
        "/game/" +
          ...
          "/gameManagement/abilities/skills/new",
        () => {}
      );
    },
    getCustomFeatures() {
      var targetURL =
        "api/api/v1/scenario/" + this.$store.getters.getScenarioKey + "/skill";
      axios
        .get(targetURL, {
    ...
    showFeatureDialog(name) {
      ...
      axios
        ...
        .then(response => {
          ...
          this.abilityScore = response.data[0].abilityScore;
          this.dialog = true;
        })
        .catch(error => {
          if (error.response.status === 401) {
